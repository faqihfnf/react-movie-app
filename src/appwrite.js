...
export const updateSearchCount = async (searchTerm, movie) => {
  ...
  try {
    ...
    //2. if it does, update the search count by 1
    if (result.documents.length > 0) {
      const doc = result.documents[0];
      await database.updateDocument(DATABASE_ID, COLLECTION_ID, doc.$id, { count: doc.count + 1 });
    } else {
      //3. if it doesn't, create a new document in the database with the search term and search count of 1
    ...
    console.log(`Error checking if search term exists: ${error}`);
  }
};
